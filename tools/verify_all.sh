...
step "1/12 الفحص البنيوي لملفات Kotlin"
python3 tools/validate_structure.py IndoLearn/app/src/main/java || fail=1

step "2/12 عدد وسائط بواني الكيانات"
python3 tools/validate_arity.py || fail=1

step "3/12 المراجع المتقاطعة (DAO / Repository / ViewModel / Navigation)"
python3 tools/validate_refs.py || fail=1

step "4/12 سلامة بيانات المحتوى"
python3 tools/validate_seed.py || fail=1

step "5/12 محاكاة قاعدة البيانات (هل تصل البيانات للشاشات؟)"
python3 tools/simulate_db.py || fail=1

step "6/12 نقاء اللغة"
python3 tools/validate_language.py || fail=1

step "7/12 تدقيق المنهج (هل عنوان الدرس يطابق محتواه؟)"
python3 tools/validate_curriculum.py || fail=1

step "8/12 تدقيق المحتوى التعليمي (تصنيف، تكرار، أمثلة)"
python3 tools/audit_content.py --strict || fail=1

step "9/12 عقد ترحيل Room (Entity ↔ SQL ↔ Database version)"
python3 tools/validate_migrations.py || fail=1

step "10/12 اختبار محلل Markdown على ملفات الموسوعة (تنفيذ فعلي)"
...
step "11/12 اختبار SeedManager (تنفيذ فعلي)"
if [ -x "/tmp/jvenv/lib/python3.11/site-packages/jdk4py/java-runtime/bin/java" ] || command -v java >/dev/null 2>&1; then
...
step "12/12 اختبارات محرك التعلم (تنفيذ فعلي)"
